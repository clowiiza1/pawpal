  ...
  return (
    <div className="min-h-screen bg-pr py-10"> {/* Using custom colors defined in your Tailwind config */}
      <div className="max-w-7xl mx-auto px-4">
        <h1 className="text-4xl font-bold text-center text-sc mb-8">Cute Kittens & Cats</h1> {/* Custom color for text */}
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {animals.length > 0 ? (
            animals.map((animal) => (
              <CatCard key={animal.id} cat={animal} /> // Pass the animal object as props
            ))
          ) : (
